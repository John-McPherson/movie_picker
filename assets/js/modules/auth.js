...
import { popWatchList } from "./watchlist.js";
...
export async function auth() {
	await getSecrets();
	let requestToken = await dataHandler(
		new Request(
			`https://api.themoviedb.org/3/authentication/token/new?api_key=${apiKey}`
		)
	);
	let authenticateUrl = ` https://www.themoviedb.org/authenticate/${
...
export async function createSession() {
	...
	if (session.success) {
		let requests = [
			{
				name: "movies",
				url: `https://api.themoviedb.org/3/account/${accountId}/watchlist/movies?api_key=${apiKey}&session_id=${session.session_id}`,
			},
			{
				name: "sightandsound",
				url: `https://api.themoviedb.org/3/list/8255505?api_key=${apiKey}`,
			},
			{
				name: "johnswatchlist",
				url: `https://api.themoviedb.org/3/list/8263553?api_key=${apiKey}`,
			},
		];

		requests.forEach(async (request) => {
			let options = await prepRequest(request.url);
			await dataHandler(
				new Request(
					`./assets/phpscripts/updatedata.php?fileName=${request.name}`
				),
				options
			);
		});
...
async function prepRequest(url) {
	...
	if (data.items) {
		data = await processData(data.items);
	} else {
...
async function processData(movies) {
...
async function populateStreaming(movieID) {
	const streamingServices = await dataHandler(
		`https://api.themoviedb.org/3/movie/${movieID}/watch/providers?api_key=${apiKey}&watch_region=GB&watch_monetization_types=flatrate`
	);
	if (
		!streamingServices.results ||
		!streamingServices.results.GB ||
		!streamingServices.results.GB.flatrate
	) {
		return [];
	}
	return streamingServices.results.GB.flatrate.map(
